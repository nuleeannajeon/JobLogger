...
function Wishlists(props) {
    ...
    return (
        <div className="content">
            <h2 className="overview-title" style={{ fontFamily: 'Varela Round' }}>
                {topicId.charAt(0).toUpperCase() + topicId.slice(1)}
            </h2>

            {dataListByType
                ? dataListByType.map((posting) => (
                      <div className="media" key={posting._id} id={posting.color}>
                          <div className="media-body">
                              <button className="box-delete-button" onClick={() => handleDelete(posting._id)}>
                                  <i className="fas fa-trash-alt"></i>
                              </button>
                              <ExistingPostModal data={posting} rerender={props.rerender} />
                              <img
                                  src={posting.companyLogoImage ? posting.companyLogoImage : companyLogo}
                                  alt={posting.company ? posting.company : ''}
                              />
                              <span style={{ fontFamily: 'Varela Round' }}>
                                  {posting.company ? posting.company : 'No defined company'}
                              </span>
                              <div style={{ paddingTop: '15px' }}>
                                  <h6 style={{ display: 'inline-block' }}>
                                      {posting.title ? posting.title : 'No defined Position'}
                                  </h6>
                                  <p style={{ color: '#4d4d4d' }}>
                                      {posting.location ? posting.location : 'No Location available'}, $
                                      {posting.salary ? posting.salary : '....'}/monthly
                                  </p>
                              </div>
                          </div>
                      </div>
